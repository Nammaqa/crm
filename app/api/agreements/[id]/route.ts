...
export async function PUT(
  ...
  try {
    ...
    const clientName = formData.get('clientName') as string || existingAgreement.clientName;
    const employeeName = formData.get('employeeName') as string;
    const employeeId = formData.get('employeeId') as string;
    const type = formData.get('type') as string || existingAgreement.type;
    const startDate = formData.get('startDate') as string || existingAgreement.startDate.toISOString();
    const endDate = formData.get('endDate') as string || existingAgreement.endDate.toISOString();
    const file = formData.get('fileUpload') as File | null;

    // Conditional validation only if type is being changed
    if (type !== existingAgreement.type) {
      // If changing to SOW or PO, validate employee info
      if ((type === 'SOW' || type === 'PO') &&
        (!employeeName && !existingAgreement.employeeName) ||
        (!employeeId && !existingAgreement.employeeId)) {
        return NextResponse.json(
          {
            success: false,
    ...
    if (formData.has('employeeId')) updateData.employeeId = employeeId || null;
    ...
    if (file && file.size > 0) updateData.fileUpload = fileUploadPath;
