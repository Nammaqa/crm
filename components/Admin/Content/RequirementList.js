...
export default function AgreementList() {
  ...
  // Fetch requirements – dependency array is always [] (stable)
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        setData({
          requirement: Array.isArray(jsonRequirements) ? jsonRequirements : [],
        });
      } catch (err) {
  ...
  const SecureDocumentViewer = ({ fileUrl, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
        <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl h-[690px] flex flex-col">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <section className="mb-6">
                  <h3 className="uppercase tracking-wide text-xs font-semibold text-gray-500 mb-3">
                    Basic Information
                  </h3>
                  <div className="grid grid-cols-2 gap-y-3 gap-x-4 border border-gray-100 bg-gray-50 rounded-lg p-4">
                    ...
                    <LabelValue
                      label="Notice Period"
                      value={requirement.noticePeriod}
                    />
                    <LabelValue
                      label="Number of Positions"
                      value={requirement.positions}
                    />
                    <LabelValue
                      label="Position Type"
                      value={requirement.closePositions}
                    />
                    <LabelValue
                      label="Budget"
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 gap-y-3 border border-gray-100 bg-gray-50 rounded-lg p-4">
                    <LabelValue
                      ...
                      value={requirement.primarySkills || "Not specified"}
                    />
                    <LabelValue
                      label="Secondary Skills"
                      value={requirement.secondarySkills || "Not specified"}
                    />
                  </div>
                </section>
              </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="px-3 py-2.5 text-left font-semibold text-gray-700 text-xs uppercase tracking-wide">
                Req ID
              </th>
              <th className="px-3 py-2.5 text-left font-semibold text-gray-700 text-xs uppercase tracking-wide">
                Name
              ...
              <th className="px-3 py-2.5 text-center font-semibold text-gray-700 text-xs uppercase tracking-wide">
                JD File
              </th>
              <th className="px-3 py-2.5 text-center font-semibold text-gray-700 text-xs uppercase tracking-wide">
                Action
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {currentItems.map((row, idx) => (
              <tr key={idx} className="hover:bg-gray-50 transition-colors">
                <td className="px-3 py-2.5 font-semibold text-gray-900 text-sm">
                  {row.requirementId}
                </td>
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b sticky top-0 z-10 shadow-sm">
        <div className="px-6 py-4 max-w-[1400px] mx-auto flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Requirement List
            </h1>
            <p className="text-gray-500 text-sm mt-0.5">
              Manage and view all job requirements
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2">
            ...
            <div className="flex items-center justify-center md:justify-start gap-2 bg-indigo-50 px-3.5 py-2 rounded border border-indigo-100">
